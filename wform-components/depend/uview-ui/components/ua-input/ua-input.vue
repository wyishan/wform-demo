	...
	>
		...
		<view class="u-input__right-icon u-flex">
			<view
				...
				@tap="onClear"
				v-show="clearableCom && valueCom != '' && focused"
			>
				<ua-icon size="32" name="close-circle-fill" color="#c0c4cc" />
			</view>
			<view
				class="u-input__right-icon__clear u-input__right-icon__item"
			...
			>
				<ua-icon name="arrow-down-fill" size="26" color="#c0c4cc"></ua-icon>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		handleInput(event) {
			...
			// 判断是否去除空格
			if (this.trim) value = $tool.trim(value);
			// vue 原生的方法 return 出去
			...
			setTimeout(() => {
				...
				// #ifdef MP-TOUTIAO
				if ($tool.trim(value) == this.lastValue) return;
