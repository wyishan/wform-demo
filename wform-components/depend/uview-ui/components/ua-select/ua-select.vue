<template>
	<view class="u-select">
		...
		<ua-popup :blur="blur" :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="popupValue" length="auto" :safeAreaInsetBottom="safeAreaInsetBottom" @close="close" :z-index="uZIndex">
			<view class="u-select">
...
<script>
  import UaPopup from "../ua-popup/ua-popup.vue"
  ...
  emits: ["update:modelValue", "input", "confirm"],
	...
	methods: {
		...
		setDefaultSelector() {
			...
			this.lastSelectIndex = $tool.deepClone(this.defaultSelector);
		},
		// 计算列数
		setColumnNum() {
			// 单列的列数为1
