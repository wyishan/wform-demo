<template>
	<view class="u-upload" v-if="!disabled">
		<view
			...
			:key="index"
			:style="{
				width: $tool.addUnit(width),
				height: $tool.addUnit(height)
			}"
		>
			<view
				v-if="deletable"
			...
			>
				<ua-icon class="u-icon" :name="delIcon" size="20" :color="delColor"></ua-icon>
			</view>
			<!-- <view
		...
		<view style="display: inline-block;" @tap="selectFile" v-if="maxCount > lists.length">
			...
			>
				<ua-icon name="plus" class="u-add-btn" size="40"></ua-icon>
...
<script>
  ...
  import UaIcon from '../ua-icon/ua-icon.vue'
  import UaLineProgress from '../ua-line-progress/ua-line-progress'
	...
	methods: {
		...
		async uploadFile(index = 0) {
			...
			if (this.beforeUpload && typeof this.beforeUpload === "function") {
				...
				// 非页面的this，所以这里需要往上历遍，一直寻找到最顶端的$parent，这里用了$tool.$parent.call(this)
				// 明白意思即可，无需纠结$tool.$parent.call(this)的细节
				// let beforeResponse = this.beforeUpload.bind($tool.$parent.call(this))(index, this.lists);
				let beforeResponse = undefined;
				// 判断是否返回了promise
			...
			const task = uni.uploadFile({
				...
				success: res => {
					...
					let data = this.toJson && $tool.test.jsonString(res.data) ? JSON.parse(res.data) : res.data;
